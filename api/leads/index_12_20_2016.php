			...
			if($cursor->count() == 0){
				...
				foreach (iterator_to_array($cursor) as $doc) {
					...
					if(!empty($_GET["reportsStatus"])){
				        ...
				        // echo $state .'</br>';
				        if($result['reportsLeadSource'] == 'ANY'){
					       	if(($result['reportsState'] == "ANY" ) || ($result['reportsState'] == strtoupper($state)) ){ 
					        	// var_dump($state);
					        	// $isItem = true;
				        ...
				        // var_dump($leadsource);
				        if(($result['reportsState'] != "ANY" ) || ($result['reportsLeadSource'] == $leadsourceId) ){ 
				        	$isItem = true;
				        }
				...
				var_dump($x++);
				if(!empty($_GET["reportsStatus"]) && $_GET["reportsState"] != 'ANY'){
					$result['total'] = $x;
				}else{
					$result['total'] = $cursor->count();
				}
